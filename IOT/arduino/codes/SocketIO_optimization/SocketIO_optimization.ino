...
float touchPos(float distance, TouchGroup* t1, TouchGroup* temp){
  ...
  if ((*t1).bottom > distance){
    if ((*t1).top - distance > errorRange){ // (bottom보다 작아서) t1 터치 그룹의 오차 범위를 넘는다면 temp에 추가한다.
    ...
    if (distance - (*t1).bottom > errorRange){ // distance가 (top보다 커서) t1 터치 그룹의 오차 범위를 넘는다면 temp에 등록한다.
      addDataTemp(temp, distance);
    }else{  // 그렇지 않고 오차 범위 내라면 t1에 거리를 추가한다.
      (*t1).cnt += 1;
      (*t1).sum += distance;
