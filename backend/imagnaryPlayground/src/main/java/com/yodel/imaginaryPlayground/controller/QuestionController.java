...
public class QuestionController {
    ...
    public Map<String, Object> deleteQuestion(
            @PathVariable @ApiParam(value = "질문삭제에 필요한 question의 id값과 user_id를 전송한다.", required = true)
            Map<String, Integer> map){
        Map<String, Object> result = new HashMap<>();
        //이후 토큰의 id와 user_id값을 비교한다.
        try {
            int res = questionService.deleteQuestion(map.get("id"));
            if(res == 1){
                result.put("status", success);
                result.put("data", questionService.lookupAllQuestion(0));
            }else{
    ...
    public Map<String, Object> lookupAllQuestion(
        ...
        try {
            if(map.get("key") == null || map.get("key").trim() == null){ //검색값이 없는 경우
                if(map.get("qna_type") == null || Integer.parseInt(map.get("qna_type")) == 0){ //문의유형이 0인 경우
                    questionList = questionService.lookupAllQuestion(Integer.parseInt(map.get("page")));
                }else{ //만약 문의유형이 0이 아닌 경우
                    questionList = questionService.lookupAllQuestionWithQuestionType(Integer.parseInt(map.get("page")), Integer.parseInt(map.get("qna_type")));
                }
            }else { //검색값이 있는 경우
                if(map.get("qna_type") == null || Integer.parseInt(map.get("qna_type")) == 0){ //문의유형이 0인 경우
                    questionList = questionService.searchAllQuestion(map);
                }else{ //만약 문의유형이 0이 아닌 경우
                    questionList = questionService.searchAllQuestionWithQuestionType(map);
                }
            }
    ...
    public Map<String, Object> lookupAllUser(@PathVariable int id){
        Map<String, Object> result = new HashMap<>();
        ...
        try {
            ...
            if(question != null){
                ...
                if(question.getCompleted() == 1){
                    data.put("answer",answerService.detailAnswer(question.getId()));
                }
            }else{
                result.put("status", fail);
            }
